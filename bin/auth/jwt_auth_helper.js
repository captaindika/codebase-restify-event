...
const queryUser = require('../modules/notification/repositories/queries/query_handler');
...
const verifyToken = async (req, res, next) => {
  ...
  const user = await queryUser.checkUserById({userId: userId});
  if (user.err) {
    result.err = new UnauthorizedError('Invalid token!');
    wrapper.response(res, 'fail', result, 'Invalid token!', ERROR.UNAUTHORIZED);
  }
  req.userId = userId;
  next();
};
